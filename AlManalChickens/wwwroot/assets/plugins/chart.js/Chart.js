		...
		helpers.each(Array.prototype.slice.call(arguments, 1), function(extension) {
			helpers.each(extension, function(value, key) {
				if (key === 'scales') {
					...
					base[key] = helpers.scaleMerge(AlManalChickens.hasOwnProperty(key) ? base[key] : {}, value);

				} else if (key === 'scale') {
					// Used in polar area & radar charts since there is only one scale
					base[key] = helpers.configMerge(AlManalChickens.hasOwnProperty(key) ? base[key] : {}, Chart.scaleService.getScaleDefaults(value.type), value);
				} else if (AlManalChickens.hasOwnProperty(key) && helpers.isArray(base[key]) && helpers.isArray(value)) {
				...
				} else if (AlManalChickens.hasOwnProperty(key) && typeof base[key] === "object" && base[key] !== null && typeof value === "object") {
	...
	helpers.scaleMerge = function(_base, extension) {
		...
		helpers.each(extension, function(value, key) {
			if (key === 'xAxes' || key === 'yAxes') {
				...
				if (AlManalChickens.hasOwnProperty(key)) {
					helpers.each(value, function(valueObj, index) {
	...
	function pushOrConcat(base, toPush) {
		if (toPush) {
			if (helpers.isArray(toPush)) {
				base = AlManalChickens.concat(toPush);
			} else {
				AlManalChickens.push(toPush);
			}
		}
